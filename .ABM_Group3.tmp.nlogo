...
patches-own [waste_visualize_v2]
...
to setup-rec_companies
  ask n-of number_rec_companies ( patches with [pcolor = blue ]) [sprout-rec_companies 1]
    ...
    if number_rec_companies = 1[ set color red]
    if number_rec_companies = 2 [
      ask rec_company 231 [set color red]
      ask rec_company 232 [set color yellow]
    ]
    if number_rec_companies = 3 [
      ask rec_company 231 [set color red]
      ask rec_company 232 [set color yellow]
      ask rec_company 233 [set color violet]
      ]
    if number_rec_companies = 4 [
      ask rec_company 231 [set color red]
      ask rec_company 232 [set color yellow]
      ask rec_company 233 [set color violet]
      ask rec_company 234 [set color black]
  ]]
end
to setup_patches
  ask patches [
...
to visualize ;; color background of turtles
  let sumofwaste sum [waste] of (turtle-set singles olds families couples )
  let sumofwastev sum [waste_visualize] of (turtle-set singles olds families couples )
  while [0.5 * sumofwaste > sumofwastev] [


    ;; Necessary lists for calculations regarding Rec1
    let distancetorec1 (turtle-set singles olds families couples ) with [waste_visualize = 0 and pcolor = grey]
    let hhofrec1 (turtle-set singles olds families couples ) with [pcolor = red]
    let sumofhhrec1 sum [waste_visualize] of hhofrec1

    while [(sumofwaste * [contract] of rec_company 231) > sumofhhrec1] [
      ask distancetorec1 with-min [distance rec_company 231] [
        set pcolor red
        set waste_visualize waste]
      set distancetorec1 (turtle-set singles olds families couples ) with [waste_visualize = 0 and pcolor = grey]
      set sumofhhrec1 sum [waste_visualize] of hhofrec1
  ]
    set sumofwastev sum [waste_visualize] of (turtle-set singles olds families couples )
  ]
end

to visualize_v2 ;; color patches
  ask patches with [pcolor = black or pcolor = red or pcolor = yellow or pcolor = violet ] [ set pcolor grey]
  let greypatches count patches with [pcolor = grey]
  let redpatches greypatches * [contract] of rec_company 231
  let yellowpatches greypatches * [contract] of rec_company 232
  let violetpatches greypatches * [contract] of rec_company 233
  let blackpatches greypatches * [contract] of rec_company 234
  while [greypatches > 2] [
    let distancelist patches with [pcolor = grey]
    if redpatches >= count patches with [pcolor = red] [ ask distancelist with-min [distance rec_company 231] [ set pcolor red
      ]
    ]
    if number_rec_companies >= 2[
    if yellowpatches >= count patches with [pcolor = yellow] [ ask distancelist with-min [distance rec_company 232] [ set pcolor yellow]
    ]]
    if number_rec_companies >= 3[
    if violetpatches >= count patches with [pcolor = violet] [ ask distancelist with-min [distance rec_company 233] [ set pcolor violet]
    ]]
     if number_rec_companies >= 4[
    if violetpatches >= count patches with [pcolor = black] [ ask distancelist with-min [distance rec_company 233] [ set pcolor black]
    ]]
    set greypatches count patches with [pcolor = grey]
  ]
end
